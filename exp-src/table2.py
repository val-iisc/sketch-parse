...
import caffe
import torch
import resnet_dilated_frozen_r5_A #TODO
import resnet_dilated_frozen_r5_B #TODO
import resnet_dilated_frozen_r5_C #TODO
import resnet_dilated_frozen_r5_D #TODO
import resnet_dilated_frozen_r5_E #TODO
...
model_A = getattr(resnet_dilated_frozen_r5_A,'Res_Deeplab')() #TODO
model_B = getattr(resnet_dilated_frozen_r5_B,'Res_Deeplab')() #TODO
model_C = getattr(resnet_dilated_frozen_r5_C,'Res_Deeplab')() #TODO
model_D = getattr(resnet_dilated_frozen_r5_D,'Res_Deeplab')() #TODO
model_E = getattr(resnet_dilated_frozen_r5_E,'Res_Deeplab')() #TODO
...
prefix_A= 'model_r5_C0_17000.pth'
prefix_B= 'model_r5_C1_19000.pth'
prefix_C= 'model_r5_C2_13000.pth'
prefix_D= 'model_r5_C3_14000.pth'
prefix_E= 'model_r5_C4_19000.pth'
...
for iter in range(1):
	...
	class_ = class_split[0]
 	selector = int(class_split[1])
	    ...
	    selector_pred = dict_pred[i]
            ...
            output_E = model_E([Variable(torch.from_numpy(img[np.newaxis, :].transpose(0,3,1,2)).float(),volatile=True).cuda(),selector_pred])
	    ...
	    output_E = merge_parts(np.argmax(interp(output_E[3]).cpu().data[0].numpy().transpose(1,2,0),axis =2),selector_pred)
    ...
    print 'A', np.sum(np.asarray(pytorch_list_A))/len(pytorch_list_A),'per class', class_ious_A
    print 'B', np.sum(np.asarray(pytorch_list_B))/len(pytorch_list_B),'per class', class_ious_B
    print 'C', np.sum(np.asarray(pytorch_list_C))/len(pytorch_list_C),'per class', class_ious_C
    print 'D', np.sum(np.asarray(pytorch_list_D))/len(pytorch_list_D),'per class', class_ious_D
    print 'E', np.sum(np.asarray(pytorch_list_E))/len(pytorch_list_E),'per class', class_ious_E
