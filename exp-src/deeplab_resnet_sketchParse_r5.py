...
class PoseModule(nn.Module):
    def __init__(self, inPlanes):
        ...
        self.PoseC1 = nn.Sequential(
                nn.Conv2d(inPlanes,  128, kernel_size=3, stride=2, padding=1, dilation=2, bias=True),
                nn.ReLU(),
...
class MS_Deeplab(nn.Module):
    def __init__(self,block):
        ...
        self.pose_r0 = PoseModule(5)
        self.pose_r1 = PoseModule(5)
        self.pose_r2 = PoseModule(7)
        self.pose_r3 = PoseModule(5)
        self.pose_r4 = PoseModule(9)
    ...
    def forward(self,x):
        ...
        if x[1] == 0:
            out.append(self.pose_r0(temp2))
        if x[1] == 1:
            out.append(self.pose_r1(temp2))
        if x[1] == 2:
            out.append(self.pose_r2(temp2))
        if x[1] == 3:
            out.append(self.pose_r3(temp2))
        if x[1] == 4:
            out.append(self.pose_r4(temp2))
