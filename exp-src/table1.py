 	...
 	selector = int(class_split[1])
        ...
        for i in img_list:
           ...
           # img = cv2.erode(img[:,:,0],kernel,iterations = 1)
            ...
            output_A = model_A(Variable(torch.from_numpy(img[np.newaxis, :].transpose(0,3,1,2)).float(),volatile=True).cuda())
            output_B = model_B(Variable(torch.from_numpy(img[np.newaxis, :].transpose(0,3,1,2)).float(),volatile=True).cuda())
            output_C = model_C(Variable(torch.from_numpy(img[np.newaxis, :].transpose(0,3,1,2)).float(),volatile=True).cuda())
            output_D = model_D(Variable(torch.from_numpy(img[np.newaxis, :].transpose(0,3,1,2)).float(),volatile=True).cuda())
	    ...
	    output_A = merge_parts(np.argmax(interp(output_A[3]).cpu().data[0].numpy().transpose(1,2,0),axis =2),selector)
	    output_B = merge_parts(np.argmax(interp(output_B[3]).cpu().data[0].numpy().transpose(1,2,0),axis =2),selector)
	    output_C = merge_parts(np.argmax(interp(output_C[3]).cpu().data[0].numpy().transpose(1,2,0),axis =2),selector)
	    output_D = merge_parts(np.argmax(interp(output_D[3]).cpu().data[0].numpy().transpose(1,2,0),axis =2),selector)
	    output_E = merge_parts(np.argmax(interp(output_E[3]).cpu().data[0].numpy().transpose(1,2,0),axis =2),selector)
